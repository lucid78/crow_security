...
bool Database::Initialize(const char* infoString)
{
    ...
    if (m_stDatabaseCount++ == 0)
    {
        ...
        {
            cerr << "Database::Initialize - Used MySQL library isn't thread-safe." << endl;
            return false;
        }
    }
    ...
    if (!pMyqlInit)
    {
        cerr << "Database::Initialize - Could not initialize Mysql connection" << endl;
        return false;
    }
    ...
    {
        cerr << "Database::Initialize - Bad infoString, format should be 'host;port;user;pw;dbname'." << endl;
        return false;
    }
    ...
    if (m_pMYSQL)
    {
        ...
        {
            cerr << "Your MySQL is out of date. Your have " + to_string(MYSQL_VERSION_ID) + " when a minimum of " + to_string(minMysqlVersion) + " is required." << endl;
            return false;
        }
    ...
    {
        cerr << "Could not connect to MySQL database " + strDbName + " at " + strHost << endl;
...
void Database::WorkerThread()
{
    ...
    cout << "Database::WorkerThread end." << endl;
}
...
bool Database::RawMysqlQueryCall(const std::string strQuery, const bool bDeleteGatheredData)
{
    ...
    {
        cerr << "SQL Error: " << string(mysql_error(m_pMYSQL)) << endl;
        cerr << "Query: " << strQuery << endl;
        return false;
    }
...
void Database::CallbackResult(const uint64 id, std::shared_ptr<CallbackQueryObj::ResultQueryHolder> result)
{
    ...
    {
        cout << "Id already exists, deleting old one." << endl;
